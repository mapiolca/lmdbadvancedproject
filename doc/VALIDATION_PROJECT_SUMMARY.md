...
Le correctif émet directement le bloc, sans changer les calculs ni le placement responsive. La suite corrigée passe sur core20/core24 avec les configurations `classic` et `phone` : un bloc, cinq tuiles, aucun contenu laissé dans le tampon inutilisé. Elle capture aussi l’absence de sortie sans droit, en édition ou module désactivé, et l’avertissement lors d’une erreur SQL. Ce sont des tests PHP simulés ; le correctif n’a pas été déployé ni vérifié sur le téléphone de l’utilisateur. Le contrôle navigateur décrit ci-dessus reste celui de la fixture initiale.
