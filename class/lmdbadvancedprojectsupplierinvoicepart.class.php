...
/**
 * Supplier invoice line part allocated to a project.
 */
class LmdbAdvancedProjectSupplierInvoicePart extends CommonObject
{
	public $element = 'lmdbadvancedproject_supplier_invoice_parts';
	public $table_element = 'lmdbadvancedproject_supplier_invoice_parts';
	public $picto = 'bill';
	...
	/**
	 * Return a link to the source supplier invoice.
	 *
	...
	public function getNomUrl($withpicto = 0, $option = '', $save_lastsearch_value = -1, $notooltip = 0)
	{
		...
		$url = DOL_URL_ROOT.'/fourn/facture/card.php?facid='.(int) $this->fk_facture_fourn;
		if ($save_lastsearch_value == 1 || ($save_lastsearch_value == -1 && !empty($_SERVER['PHP_SELF']) && preg_match('/list\.php/', $_SERVER['PHP_SELF']))) {
			$url .= '&save_lastsearch_values=1';
		}

		$label = '<u>'.$langs->trans('ShowSupplierInvoice').'</u><br><b>'.$langs->trans('Ref').':</b> '.$ref;
		$linkclose = '';
		if (empty($notooltip)) {
	...
	public function fetch($id, $user = null)
	{
		...
		$sql .= " v.fk_soc, v.fk_projet, v.fk_facture_fourn, v.fk_facture_fourn_det, v.entity, v.fk_user_author, v.fk_user_modif";
		...
		if ($resql) {
			if ($this->db->num_rows($resql)) {
				...
				$this->fk_facture_fourn = (int) $obj->fk_facture_fourn;
				$this->fk_facture_fourn_det = (int) $obj->fk_facture_fourn_det;
